...
CREATE TABLE `new_sold_tickets` (`id` integer NULL PRIMARY KEY AUTOINCREMENT, `created_at` datetime NULL, `updated_at` datetime NULL, `deleted_at` datetime NULL, `event_id` integer NULL, `buyer_id` integer NULL, `price` real NULL, `secret` text NOT NULL);
-- Copy rows from old table "sold_tickets" to new temporary table "new_sold_tickets"
INSERT INTO `new_sold_tickets` (`id`, `created_at`, `updated_at`, `deleted_at`, `event_id`, `buyer_id`, `price`, `secret`) SELECT `id`, `created_at`, `updated_at`, `deleted_at`, `event_id`, CAST(`user_id` as integer), `price`, `secret` FROM `sold_tickets`;
