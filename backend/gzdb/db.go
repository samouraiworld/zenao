...
func (g *gormZenaoDB) Checkin(pubkey string, gatekeeperID string, signature string) (*zeni.Event, error) {
	...
	roles, err := g.EntityRoles(zeni.OrgTypeUser, fmt.Sprintf("%d", dbTicket.UserID), zeni.OrgTypeEvent, fmt.Sprint(dbTicket.EventID))
	if err != nil {
		return nil, err
	}
