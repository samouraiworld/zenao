...
func (s *ZenaoServer) Participate(ctx context.Context, req *connect.Request[zenaov1.ParticipateRequest]) (*connect.Response[zenaov1.ParticipateResponse], error) {
	...
	if err := s.DB.Tx(func(db zeni.DB) error {
		...
		if err := db.Participate(req.Msg.EventId, userID, ticket.Secret()); err != nil {
			return err
		}
		...
		if err := s.Chain.Participate(req.Msg.EventId, evt.CreatorID, userID, ticket.Pubkey()); err != nil {
