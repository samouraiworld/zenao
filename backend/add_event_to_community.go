...
func (s *ZenaoServer) AddEventToCommunity(
	...
	if err := s.Chain.AddEventToCommunity(zUser.ID, req.Msg.CommunityId, req.Msg.EventId); err != nil {
	...
	for _, participant := range participants {
		if !targetIDs[participant.ID] {
			if err := s.Chain.AddMemberToCommunity(zUser.ID, req.Msg.CommunityId, participant.ID); err != nil {
